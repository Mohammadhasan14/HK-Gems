...
- **Current placeholder**: two `BufferGeometry` sources, swapped by reference in `HeroStone.tsx` based on scroll position, not regenerated per frame. Before Beat 3 (Arrival/Origin/Inhale): a purpose-built faceted gem cut (`lib/gemGeometry.ts`) — table, crown, girdle, pavilion, culet, 8-fold symmetry — standing in for the finished stone's silhouette in the hero shot. From Beat 3 onward: the original 5-stage sequence generated by slicing a unit icosahedron with planes (`lib/cutStages.ts`), unchanged. `flatShading` throughout both; `MeshTransmissionMaterial` on HIGH tier, native `meshPhysicalMaterial` transmission on LOW/STATIC (see Material target below — this placeholder already uses the Phase 1 target material)
- **Delivery format**: pre-authored `BufferGeometry`s, NOT runtime CSG — CSG can't be scrubbed against scroll position. The real modeller-authored asset can either replace `lib/gemGeometry.ts`'s single display shape directly, or (better long-term) replace the whole pair with one continuous 6-stage sequence — rough hull → the real cut → the 4 existing cut stages — so the pre-Cut hero shot and the Cut beat's own progression are the same authored asset instead of two.
